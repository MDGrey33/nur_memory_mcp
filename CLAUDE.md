...
# Reset all data (safer - uses env-reset.sh)
...
# Run migrations
docker exec -i mcp-postgres psql -U events -d events < ../implementation/mcp-server/migrations/XXX.sql
```
...
For deeper context, these can be imported when needed:
...
- **Deployment Guide**: `.claude-workspace/deployment/deployment-guide.md` - Production deployment
