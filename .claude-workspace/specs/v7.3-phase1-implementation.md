...
> **Note**: This spec is closed. Incomplete items (triplet scoring) moved to [V9](./v9-consolidation.md).
...
Each change follows: **Benchmark → Build → Compare → Decide**
...
src/services/retrieval_service.py
  └─ Add: triplet_score() function
  └─ Modify: _graph_expand() to use triplet scoring
  └─ Modify: _merge_results() to incorporate triplet scores

src/services/embedding_service.py
  └─ Add: embed_relationship() for edge embeddings (optional)
```
...
|--------|----------|--------|----------|
| Outcome pass rate | 80% | ≥80% | Must not regress |
| Found count | 4/5 | ≥4/5 | Must not regress |
| Retrieval latency p50 | TBD | <1.2x baseline | <20% slower |

**Decision**:
- ✅ **KEEP** if pass rate ≥ baseline AND latency < 1.5x baseline
- ❌ **REVERT** if pass rate drops OR latency > 2x baseline
...
1. **Phase 1**: Vector search → get candidate artifact IDs
...
```
src/services/retrieval_service.py
  └─ Modify: recall() to use two-phase approach
  └─ Add: _get_candidate_ids() helper
  └─ Modify: _graph_expand() to accept id filter
```
...
async def recall(
    ...
    **filters
    ...
    """
    Two-phase retrieval: vector search → filtered graph expansion.
    """
...
| Metric | Baseline | Target | Go/No-Go |
|--------|----------|--------|----------|
| Recall latency p50 | TBD | ≤0.7x baseline | 30% faster |
| Recall latency p95 | TBD | ≤0.8x baseline | 20% faster |
| DB queries per recall | TBD | ≤0.5x baseline | 50% fewer |
| Outcome pass rate | 80% | ≥80% | Must not regress |

**Decision**:
- ✅ **KEEP** if latency improves ≥20% AND pass rate ≥ baseline
- ❌ **REVERT** if pass rate drops >5%
- 🔄 **ITERATE** if latency doesn't improve but doesn't regress
...
src/services/event_extraction_service.py
  └─ Modify: EXTRACTION_PROMPT to allow any category
  └─ Remove: category validation/normalization

src/storage/postgres_models.py
  └─ Modify: semantic_event.category from ENUM to VARCHAR

migrations/
  └─ Add: 007_dynamic_categories.sql
```
...
| Metric | Baseline | Target | Go/No-Go |
|--------|----------|--------|----------|
| Outcome pass rate | 80% | ≥75% | Allow slight regression |
| Unique categories | 8 (fixed) | >8 | More diversity |
| Category coverage | Manual check | Reasonable | No garbage |

**Decision**:
- ✅ **KEEP** if pass rate ≥75% AND categories look reasonable
- ❌ **REVERT** if pass rate <70% OR garbage categories
- 🔄 **ITERATE** if categories too fragmented (add normalization)
